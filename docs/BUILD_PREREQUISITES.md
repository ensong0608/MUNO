...
Run Blender's dependency update from the real Blender Git checkout. The active local build workspace is now on `C:`:

```bat
cd /d "C:\DIG REPO\tools\Muno\upstream"
make.bat update
```

Do not run `make.bat update` from `C:\DIG REPO\tools\Muno\source`. The `source/` directory is generated by MUNO's overlay scripts and is not a Git working tree, so Blender's submodule/LFS update commands can fail there.

After dependency updates, regenerate the MUNO source tree:

```bat
cd /d "C:\DIG REPO\tools\Muno"
scripts\windows\overlay.bat
```

## First Build Commands To Validate

From `C:\DIG REPO\tools\Muno\source`:
...
- `C:\DIG REPO\tools\Muno\upstream\lib\windows_x64`: about 6.51 GB.
- `C:\DIG REPO\tools\Muno\source\lib\windows_x64`: about 6.51 GB after overlay copy.
